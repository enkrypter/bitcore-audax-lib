...
describe('Unit', function() {
  ...
  it('has property accesors "AUDAX", "mAUDAX", "uAUDAX", "bits", and "satoshis"', function() {
    var unit = new Unit(1.2, 'AUDAX');
    unit.AUDAX.should.equal(1.2);
    unit.mAUDAX.should.equal(1200);
    unit.uAUDAX.should.equal(1200000);
  ...
  it('a string amount is allowed', function() {
    ...
    unit = Unit.fromAUDAX('1.00001');
    unit.AUDAX.should.equal(1.00001);

    unit = Unit.fromMilis('1.00001');
    unit.mAUDAX.should.equal(1.00001);
    ...
    unit.AUDAX.should.equal(0.12285714);
  });
  ...
  it('exposes unit codes', function() {
    should.exist(Unit.AUDAX);
    Unit.AUDAX.should.equal('AUDAX');

    should.exist(Unit.mAUDAX);
    Unit.mAUDAX.should.equal('mAUDAX');
  ...
  it('exposes a method that converts to different units', function() {
    var unit = new Unit(1.3, 'AUDAX');
    unit.to(Unit.AUDAX).should.equal(unit.AUDAX);
    unit.to(Unit.mAUDAX).should.equal(unit.mAUDAX);
  ...
  it('exposes shorthand conversion methods', function() {
    var unit = new Unit(1.3, 'AUDAX');
    unit.toAUDAX().should.equal(unit.AUDAX);
    unit.toMilis().should.equal(unit.mAUDAX);
    unit.toMillis().should.equal(unit.mAUDAX);
  ...
  it('can be imported and exported from/to JSON', function() {
    var json = JSON.stringify({amount:1.3, code:'AUDAX'});
  ...
  it('fails when the unit is not recognized', function() {
    ...
    expect(function() {
      return new Unit(100, 'AUDAX').to('USD');
    }).to.throw(errors.Unit.UnknownCode);
  });
  ...
  it('fails when the exchange rate is invalid', function() {
    ...
    expect(function() {
      return new Unit(100, 'AUDAX').atRate(-123);
    }).to.throw(errors.Unit.InvalidRate);
  });
